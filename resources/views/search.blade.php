...
@section('content')
    ...
    <div class="col-md-8">
		...
		<div class="well">
       		<h4 class="text-center">There @if ($count == 0 || $count == 1){{ 'is' }} @else {{ 'are' }} @endif {{ $count }} @if ($count == 0 || $count == 1) {{ 'post' }} @else {{ 'posts' }} @endif corresponding to query <span style="background: yellow;">{{ $query }}</span></h4>
       	</div>
